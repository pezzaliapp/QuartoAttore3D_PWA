// Quarto Attore 3D — Journey-like PWA
// app.js v1.1 — WebGL→Canvas2D failover automatico
...
function startCanvas2D(){
  ...
  const P = 900;
  const parts = new Float32Array(P*3);
  for(let i=0;i<P;i++){
    parts[3*i+0]=(Math.random()-0.5)*80;
    parts[3*i+1]=Math.random()*2.5+0.2;
    parts[3*i+2]=(Math.random()-0.5)*80;
  }
  addEventListener('pointerdown', ()=>{
    const now=performance.now();
    glide = Math.min(1, glide + (now-lastTap<300 ? 0.3 : 0.15));
    lastTap=now;
  }, {passive:true});
  ...
  function frame(){
    ...
    g.addColorStop(0,'#172042'); g.addColorStop(1,'#0b1022');
    ctx.fillStyle=g; ctx.fillRect(0,0,w,h);

    // dunes (3 strati per parallax)
    function drawDune(scale, amp, yBase, colA='#EBC88C', colB='#AE8C55'){
      ctx.beginPath();
      for(let x=0;x<w;x+=2){
      ...
      const g2 = ctx.createLinearGradient(0,yBase-60*DPR,0,h);
      g2.addColorStop(0, colA); g2.addColorStop(1, colB);
      ctx.fillStyle=g2; ctx.globalAlpha=0.95; ctx.fill(); ctx.globalAlpha=1.0;
    }
    drawDune(0.6, 0.8, h*0.55);
    drawDune(0.9, 1.0, h*0.68);
    drawDune(1.3, 1.2, h*0.82);

    // traveler (cerchio luminoso)
    const cx = w*0.5, cy = h*0.6 + Math.sin(t*1.8)*6*DPR;
    ctx.beginPath(); ctx.arc(cx, cy, 14*DPR, 0, Math.PI*2);
    ctx.fillStyle='#f6edd7'; ctx.fill();
    const glow = ctx.createRadialGradient(cx,cy,0,cx,cy,26*DPR);
    glow.addColorStop(0,'rgba(255,233,196,0.25)');
    glow.addColorStop(1,'rgba(255,233,196,0)');
    ctx.fillStyle=glow; ctx.beginPath(); ctx.arc(cx,cy,26*DPR,0,Math.PI*2); ctx.fill();

    // wind particles 2D
    ctx.fillStyle='rgba(255,255,255,0.35)';
    for(let i=0;i<P;i++){
      let x = parts[3*i+0], z = parts[3*i+2];
      x += 0.12; z += 0.09;
      if (x>40) x=-40; if (z>40) z=-40;
      parts[3*i+0]=x; parts[3*i+2]=z;
      const sx = (x/80+0.5)*w;
      const sy = (0.58 + Math.sin((z+x)*0.08 + t*0.4)*0.02) * h;
      ctx.fillRect(sx, sy, 2*DPR, 2*DPR);
    }

    // avanzamento “glide” solo scenografico in 2D
    if (glide>0.0005) glide *= 0.985;
  }
  frame();
  showNote('Modalità compatibilità (Canvas2D)');
}

// ───────────────── WebGL (Three.js) con shader dune
async function startWebGL(){
  // import dinamico: meno problemi con CSP quando lanci da file-server semplici
  const threeUrl   = 'https://cdn.jsdelivr.net/npm/three@0.162.0/build/three.module.min.js';
  const orbitUrl   = 'https://cdn.jsdelivr.net/npm/three@0.162.0/examples/jsm/controls/OrbitControls.js';
  const gltfUrl    = 'https://cdn.jsdelivr.net/npm/three@0.162.0/examples/jsm/loaders/GLTFLoader.js';

  const [{default: THREE}, {OrbitControls}, {GLTFLoader}] = await Promise.all([
    import(threeUrl), import(orbitUrl), import(gltfUrl)
  ]);
  ...
  renderer.domElement.addEventListener('webglcontextlost', e=>{ e.preventDefault(); showNote('WebGL context lost'); startCanvas2D(); }, false);
  ...
  controls.minDistance = 2; controls.maxDistance = 14; controls.maxPolarAngle = Math.PI*0.55;
  ...
  try{
    duneMat = new THREE.ShaderMaterial({
      ...
      uniforms:{
        ...
        uFogNear:{value:40.0}, uFogFar:{value:180.0}
      },
      vertexShader:`
    ...
    console.warn('Shader error, switching to standard:', err);
    duneGeo.computeVertexNormals();
    dunes = new THREE.Mesh(duneGeo, new THREE.MeshStandardMaterial({ color:0xC9A873, roughness:0.95, metalness:0.0 }));
    scene.add(dunes);
    showNote('Shader non disponibile — materiale standard');
  }
  ...
  (function tick(){
    ...
    t += 0.016;
    if (duneMat && duneMat.uniforms && duneMat.uniforms.uTime){ duneMat.uniforms.uTime.value = t; }
  ...
  showNote('Modalità WebGL (Three.js)');
}
...
(async ()=>{
  try{
    ...
    if (FORCE==='webgl'){ await startWebGL(); return; }
    // tenta WebGL, se fallisce va in 2D
    await startWebGL();
  }catch(err){
    console.warn('Fallo WebGL → Canvas2D:', err);
    showNote('Switch a compatibilità (Canvas2D)');
    startCanvas2D();
  }
})();
